...
const Help = () => {
  return (
    <div className='mt-28 subtitle-animate'>
      <h2 className='mb-4 md:mb-6 text-2xl sm:text-5xl text-white title-animate'>
      ...
      <ol className='grid grid-cols-12 -mx-2.5'>
        {helps.map((help) => {
          return (
            <li
              key={help.id}
              className={`flex md:flex-row flex-col ${help.id>5?'col-span-12':help.id<3?'md:col-span-6 col-span-12':help.id==5?'xl:col-span-4 col-span-12':'xl:col-span-4 md:col-span-6 col-span-12'} m-2.5 rounded-2xl text-gray-300 bg-[#181F2F] border border-[#2A3755]`}
              style={{ animationDelay: `${0.2 + help.id * 0.1}s` }}
            >
              <div className='flex flex-col w-full'>
                <div className='flex flex-row justify-between items-center gap-4 m-5'>
                  <h4 className='px-4 py-3 text-sm sm:text-base md:text-lg lg:text-xl font-normal text-white rounded-full border border-[#2A3755]'>
                  ...
                  </h4>
                  {
                    ...
                    <div className='flex justify-center items-center w-12 h-12 md:w-14 md:h-14 px-4 py-3 rounded-full border border-[#2A3755]'>
                      <Image alt={`${help.id}Img`} src={help.img} width={50} height={50}/>
                    </div>
                  }
                </div>
                <p className='mx-5 mt-1 mb-5 text-xs sm:text-sm md:text-base lg:text-lg text-[#A4B0C8] text-justify'>
                  {help.text}
                </p>
